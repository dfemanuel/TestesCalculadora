...
@then('subtraction should be {result:f}')
def step_impl(context, precision, result):
    c = CalculadoraPrecisao(precision)
    assert c.subtracao(context.a, context.b) == result

@then('multiplication should be {result:f}')
def step_impl(context, precision, result):
    c = CalculadoraPrecisao(precision)
    assert c.multiplicacao(context.a, context.b) == result

@then('division should be {result:f}')
def step_impl(context, precision, result):
    c = CalculadoraPrecisao(precision)
    assert c.divisao(context.a, context.b) == result